...
module.exports = {
	...
	updateApplication:function(req, user, callback){
		...
		query += ' SET '
			...
			+ ',is_18="' + ((user.is_18 === 'on') ? true : false) + '"'
			+ ',attend_school="' + ((user.attend_school === 'on') ? true : false) + '"'
			+ ',school="' + user.school + '"'
			+ ',state="' + user.state + '"'
			...
			+ ',venmo="' + user.venmo + '"'
			+ ',food_vegetarian="' + ((user.food_vegetarian === 'on') ? true : false) + '"'
			+ ',food_vegan="' + ((user.food_vegan === 'on') ? true : false) + '"'
			+ ',food_glutenfree="' + ((user.food_glutenfree === 'on') ? true : false) + '"'
			+ ',food_pescetarian="' + ((user.food_pescetarian === 'on') ? true : false) + '"'
			+ ',food_dairyfree="' + ((user.food_dairyfree === 'on') ? true : false) + '"'
			+ ',food_kosher="' + ((user.food_kosher === 'on') ? true : false) + '"'
			+ ',food_halal="' + ((user.food_halal === 'on') ? true : false) + '"'
			+ ',food_other="' + user.food_other + '"'
			+ ',Github="' + user.Github + '"'
			+ ',linkedIn="' + user.linkedIn + '"'
			+ ',Dribble="' + user.Dribble + '"'
			+ ',exposure="' + user.exposure + '"'
			+ ',plan_hardware="' + ((user.plan_hardware === 'on') ? true : false) + '"'
			+ ',plan_hardware_on="' + user.plan_hardware_on + '"'
			+ ',plan_mentee="' + ((user.plan_mentee === 'on') ? true : false) + '"'
			+ ',plan_mentor="' + ((user.plan_mentor === 'on') ? true : false) + '"'
			+ ',need_partner="' + ((user.need_partner === 'on') ? true : false) + '"'
	...
	checkLogin: function(user, callback){
		var query = "SELECT password FROM applications WHERE email='"+user.email+"';";
	...
	getUser: function(req, callback){
		var query = "SELECT * FROM applications WHERE email='"+req.signedCookies.user+"';";
